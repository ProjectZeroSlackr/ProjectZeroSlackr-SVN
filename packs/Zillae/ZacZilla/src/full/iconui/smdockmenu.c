...
static void render (TWidget *this, int first, int n)
{
	...
	data->itemsrf[xi] = ttk_new_surface (data->itemheight, data->itemheight, ttk_screen->bpp);
	ttk_fillrect (data->itemsrf[xi], 0, 0, data->itemheight, data->itemheight, menu_bg_color );
    iconui_draw_small_icon (data->itemsrf[xi], (char *)data->menu[xi]->name, iofs, iofs);

	// Selected
	if (data->itemsrfI[xi]) ttk_free_surface (data->itemsrfI[xi]);
	data->itemsrfI[xi] = ttk_new_surface (data->itemheight, data->itemheight, ttk_screen->bpp);
	ttk_ap_rect (data->itemsrfI[xi], ttk_ap_getx ("menu.selbg"), 0, 0, data->itemheight, data->itemheight);
	iconui_draw_small_icon (data->itemsrfI[xi], (char *)data->menu[xi]->name, iofs, iofs);
    }
}



ttk_menu_item *smdockiconui_menu_get_item (TWidget *this, int xi) 
{
    _MAKETHIS;
...
ttk_menu_item *smdockiconui_menu_get_item_called (TWidget *this, const char *s)
{
    int xi = 0;
    _MAKETHIS;
    if (data->menu) {
...
ttk_menu_item *smdockiconui_menu_get_selected_item (TWidget *this) 
{
    _MAKETHIS;
...
void smdockiconui_menu_item_updated (TWidget *this, ttk_menu_item *p)
{
    _MAKETHIS;
    int i;
...
void smdockiconui_menu_remove_by_name (TWidget *this, const char *name)
{
    _MAKETHIS;
    int xi;
...
void smdockiconui_menu_remove_by_ptr (TWidget *this, ttk_menu_item *item) 
{
    _MAKETHIS;
...
void smdockiconui_menu_remove (TWidget *this, int xi) 
{
    _MAKETHIS;
...
void smdockiconui_menu_insert (TWidget *this, ttk_menu_item *item, int xi) 
{
    _MAKETHIS;
    
    if (xi >= data->items) {
        smdockiconui_menu_append (this, item);
        return;
    }
    ...
    smdockiconui_menu_item_updated (this, item);
    MakeVIXI (this);
    item->menudata = data;
...
void smdockiconui_menu_append (TWidget *this, ttk_menu_item *item) 
{
    _MAKETHIS;
...
void smdockiconui_menu_sort_my_way (TWidget *this, int (*cmp)(const void *, const void *))
{
    _MAKETHIS;
...
void smdockiconui_menu_sort (TWidget *this) 
{
    _MAKETHIS;
    if (data->i18nable)
        smdockiconui_menu_sort_my_way (this, sort_compare_i18n);
    else
        smdockiconui_menu_sort_my_way (this, sort_compare);
}

void smdockiconui_menu_updated (TWidget *this)
{
    ttk_menu_item *p, **q;
    _MAKETHIS;
    ...
    q = data->menu;
    while (p && p->name) {
	smdockiconui_menu_item_updated (this, p);
	p->menudata = data;
...
int smdockiconui_menu_frame (TWidget *this) 
{   
...
void smdockiconui_menu_flash (ttk_menu_item *item, int n) 
{
    item->textflash = 2*n;
}
...
TWidget *smdockiconui_new_menu_widget (ttk_menu_item *items, ttk_font font, int w, int h)
{
    ...
    ret->data = data;
    ret->focusable = 1;
    ret->draw = smdockiconui_menu_draw;
    ret->frame = smdockiconui_menu_frame;
    ret->down = smdockiconui_menu_down;
    ret->button = smdockiconui_menu_button;
    ret->scroll = smdockiconui_menu_scroll;
    ret->destroy = smdockiconui_menu_free;
...
void smdockiconui_menu_set_closeable (TWidget *this, int closeable) 
{
    _MAKETHIS;
    data->closeable = closeable;
}
void smdockiconui_menu_set_i18nable (TWidget *this, int i18nable) 
{
    _MAKETHIS;
    data->i18nable = i18nable;
}

void smdockiconui_menu_draw (TWidget *this, ttk_surface srf)
{
    _MAKETHIS;
    int i, x = this->x + (this->w - (MIN(data->visible, data->vitems))*data->itemheight)/2, y = this->y + this->h - data->itemheight;
	...
	x = this->x + (this->w - (MIN(data->visible, data->vitems))*data->itemheight)/2;
	y = this->y + this->h - data->itemheight;
	for (i = 0; i < data->items; i++)
	    smdockiconui_menu_item_updated (this, data->menu[i]);
	render (this, data->top, data->visible);
	data->epoch = ttk_epoch;
    }
	...
	if (vi == data->top + data->sel && !(data->menu[xi]->textflash % 2)) {
	    ...
	    ttk_blit_image_ex (data->itemsrfI[xi], 0, 0, data->itemheight, data->itemheight, srf, x, y);
	    selxi = xi;
	    selx = x;
	} else {
	    if (!data->itemsrf[xi]) {
	    ...
	    ttk_blit_image_ex (data->itemsrf[xi], 0, 0, data->itemheight, data->itemheight, srf, x, y);
	}
...
*/
    ...
    if (selxi >= 0) {
	    ...
	    if (data->menu[xi]->choices) {
	        ...
	        cx = x + (data->itemheight - cw)/2;
	        if (cx > (this->w - cw)) cx = this->w - cw;
	        if (cx < 0) cx = 0;
	        ttk_text (srf, data->font, cx, y - ttk_text_height(data->font) - 2, col, data->menu[xi]->choices[data->menu[xi]->choice]);
	        nw = ttk_text_width(data->font, data->menu[xi]->name);
	        nx = x + (data->itemheight - nw)/2;
	        if (nx > (this->w - nw)) nx = this->w - nw;
	        if (nx < 0) nx = 0;
	    	ttk_text (srf, data->font, nx, y - ttk_text_height(data->font)*2 - 4, col, data->menu[xi]->name);
	    } else {
	        nw = ttk_text_width(data->font, data->menu[xi]->name);
	        nx = x + (data->itemheight - nw)/2;
	        if (nx > (this->w - nw)) nx = this->w - nw;
	        if (nx < 0) nx = 0;
	    	ttk_text (srf, data->font, nx, y - ttk_text_height(data->font) - 2, col, data->menu[xi]->name);
	    }
    }
...
int smdockiconui_menu_scroll (TWidget *this, int dir)
{
    _MAKETHIS;
    int oldtop, oldsel;
    TTK_SCROLLMOD (dir, 5);
...
int smdockiconui_menu_button (TWidget *this, int button, int time)
{
    _MAKETHIS;
    ttk_menu_item *item;
    TWindow *sub;
	...
	if (item->sub == TTK_MENU_DONOTHING) {
	    ...
	    int r = -1;
	    while (ttk_windows->w->focus->draw == smdockiconui_menu_draw &&
...
int smdockiconui_menu_down (TWidget *this, int button)
{
    return TTK_EV_UNUSED;
}
...
void smdockiconui_menu_free (TWidget *this) 
{
    int i;
    _MAKETHIS;
...
void smdockiconui_menu_clear (TWidget *this) 
{
    int i;
    _MAKETHIS;
    if (data->menu) {
...
TWindow *smdockiconuix_mh_sub (ttk_menu_item *item) 
{
    TWindow *ret = ttk_new_window();

    ttk_add_widget (ret, smdockiconui_new_menu_widget (item->data, ttk_menufont,
...
void *smdockiconuix_md_sub (ttk_menu_item *submenu) 
{
    return (void *)submenu;
}
