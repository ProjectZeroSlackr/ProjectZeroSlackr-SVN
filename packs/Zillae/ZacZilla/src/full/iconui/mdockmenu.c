...
static void render (TWidget *this, int first, int n)
{
    ...
    ofs = (data->itemheight - 16) / 2;
    iofs = (data->itemheight - 16);
	...
	data->itemsrf[xi] = ttk_new_surface (data->itemheight*2, data->itemheight*2, ttk_screen->bpp);
	ttk_fillrect (data->itemsrf[xi], 0, 0, data->itemheight*2, data->itemheight*2, menu_bg_color );
    iconui_draw_icon (data->itemsrf[xi], (char *)data->menu[xi]->name, iofs, iofs);

	// Selected
	if (data->itemsrfI[xi]) ttk_free_surface (data->itemsrfI[xi]);
	data->itemsrfI[xi] = ttk_new_surface (data->itemheight*2, data->itemheight*2, ttk_screen->bpp);
	ttk_ap_rect (data->itemsrfI[xi], ttk_ap_getx ("menu.selbg"), 0, 0, data->itemheight*2, data->itemheight*2);
	iconui_draw_icon (data->itemsrfI[xi], (char *)data->menu[xi]->name, iofs, iofs);
    }
}



ttk_menu_item *mdockiconui_menu_get_item (TWidget *this, int xi) 
{
    _MAKETHIS;
...
ttk_menu_item *mdockiconui_menu_get_item_called (TWidget *this, const char *s)
{
    int xi = 0;
    _MAKETHIS;
    if (data->menu) {
...
ttk_menu_item *mdockiconui_menu_get_selected_item (TWidget *this) 
{
    _MAKETHIS;
...
void mdockiconui_menu_item_updated (TWidget *this, ttk_menu_item *p)
{
    _MAKETHIS;
    int i;
...
void mdockiconui_menu_remove_by_name (TWidget *this, const char *name)
{
    _MAKETHIS;
    int xi;
...
void mdockiconui_menu_remove_by_ptr (TWidget *this, ttk_menu_item *item) 
{
    _MAKETHIS;
...
void mdockiconui_menu_remove (TWidget *this, int xi) 
{
    _MAKETHIS;
...
void mdockiconui_menu_insert (TWidget *this, ttk_menu_item *item, int xi) 
{
    _MAKETHIS;
    
    if (xi >= data->items) {
        mdockiconui_menu_append (this, item);
        return;
    }
    ...
    mdockiconui_menu_item_updated (this, item);
    MakeVIXI (this);
    item->menudata = data;
...
void mdockiconui_menu_append (TWidget *this, ttk_menu_item *item) 
{
    _MAKETHIS;
...
void mdockiconui_menu_sort_my_way (TWidget *this, int (*cmp)(const void *, const void *))
{
    _MAKETHIS;
...
void mdockiconui_menu_sort (TWidget *this) 
{
    _MAKETHIS;
    if (data->i18nable)
        mdockiconui_menu_sort_my_way (this, sort_compare_i18n);
    else
        mdockiconui_menu_sort_my_way (this, sort_compare);
}

void mdockiconui_menu_updated (TWidget *this)
{
    ttk_menu_item *p, **q;
    _MAKETHIS;
    ...
    q = data->menu;
    while (p && p->name) {
	mdockiconui_menu_item_updated (this, p);
	p->menudata = data;
...
int mdockiconui_menu_frame (TWidget *this) 
{   
...
void mdockiconui_menu_flash (ttk_menu_item *item, int n) 
{
    item->textflash = 2*n;
}


TWidget *mdockiconui_new_menu_widget (ttk_menu_item *items, ttk_font font, int w, int h)
{
	int th;
    ttk_menu_item *p;
    ...
    ret->data = data;
    ret->focusable = 1;
    ret->draw = mdockiconui_menu_draw;
    ret->frame = mdockiconui_menu_frame;
    ret->down = mdockiconui_menu_down;
    ret->button = mdockiconui_menu_button;
    ret->scroll = mdockiconui_menu_scroll;
    ret->destroy = mdockiconui_menu_free;
...
void mdockiconui_menu_set_closeable (TWidget *this, int closeable) 
{
    _MAKETHIS;
    data->closeable = closeable;
}
void mdockiconui_menu_set_i18nable (TWidget *this, int i18nable) 
{
    _MAKETHIS;
    data->i18nable = i18nable;
}

void mdockiconui_menu_draw (TWidget *this, ttk_surface srf)
{
    _MAKETHIS;
    int i;
    int x = this->x + (this->w - ((MIN(data->visible, data->vitems))+2)*data->itemheight)/2;
    int y = this->y + this->h - data->itemheight;
    int y1 = this->y + this->h - (data->itemheight + data->itemheight/2);
    int y2 = this->y + this->h - data->itemheight*2;
    int ofs = (data->itemheight - 16) / 2;
    int nivis = 0;
    int spos, sheight;
    int vi, xi;
    int selxi = -1, selx = -1;
	...
	data->visible = this->w / 18;
	data->itemheight = this->w / data->visible;
	data->visible -= 2;
	x = this->x + (this->w - ((MIN(data->visible, data->vitems))+2)*data->itemheight)/2;
	y = this->y + this->h - data->itemheight;
	y1 = this->y + this->h - (data->itemheight + data->itemheight/2);
	y2 = this->y + this->h - data->itemheight*2;
	for (i = 0; i < data->items; i++)
	    mdockiconui_menu_item_updated (this, data->menu[i]);
	render (this, data->top, data->visible);
	data->epoch = ttk_epoch;
    }
    ...
    ttk_ap_rect (srf, ttk_ap_getx ("menu.bg"), this->x, this->y, this->x + this->w, this->y + this->h);
	...
	if (vi == data->top + data->sel && !(data->menu[xi]->textflash % 2)) {
	    ...
	    ttk_blit_image_ex (data->itemsrfI[xi], 0, 0, data->itemheight*2, data->itemheight*2, srf, x, y2);
	    selxi = xi;
	    selx = x;
	    x += data->itemheight*2;
	} else if (vi == (data->top + data->sel - 1) || vi == (data->top + data->sel + 1)) {
	    if (!data->itemsrf[xi]) {
		fprintf (stderr, "Null pointer (N)\n");
		abort();
	    }
	    
	    ssrf = ttk_scale_surface(data->itemsrf[xi], 0.75);
	    ttk_blit_image_ex (ssrf, 0, 0, data->itemheight + data->itemheight/2, data->itemheight + data->itemheight/2, srf, x, y1);
	    ttk_free_surface(ssrf);
	    x += data->itemheight + data->itemheight/2;
	} else {
	    if (!data->itemsrf[xi]) {
		fprintf (stderr, "Null pointer (N)\n");
		abort();
		...
		ssrf = ttk_scale_surface(data->itemsrf[xi], 0.5);
	    ttk_blit_image_ex (ssrf, 0, 0, data->itemheight, data->itemheight, srf, x, y);
	    ttk_free_surface(ssrf);
	    x += data->itemheight;
	}
...
*/
    ...
    if (selxi >= 0) {
	    ...
	    if (data->menu[xi]->choices) {
	    	cw = ttk_text_width (data->font, data->menu[xi]->choices[data->menu[xi]->choice]);
	        cx = x + (data->itemheight*2 - cw)/2;
	        if (cx > (this->w - cw)) cx = this->w - cw;
	        if (cx < 0) cx = 0;
	        ttk_text (srf, data->font, cx, y2 - ttk_text_height(data->font) - 2, col, data->menu[xi]->choices[data->menu[xi]->choice]);
	        nw = ttk_text_width(data->font, data->menu[xi]->name);
	        nx = x + (data->itemheight*2 - nw)/2;
	        if (nx > (this->w - nw)) nx = this->w - nw;
	        if (nx < 0) nx = 0;
	    	ttk_text (srf, data->font, nx, y2 - ttk_text_height(data->font)*2 - 4, col, data->menu[xi]->name);
	    } else {
	        nw = ttk_text_width(data->font, data->menu[xi]->name);
	        nx = x + (data->itemheight*2 - nw)/2;
	        if (nx > (this->w - nw)) nx = this->w - nw;
	        if (nx < 0) nx = 0;
	    	ttk_text (srf, data->font, nx, y2 - ttk_text_height(data->font) - 2, col, data->menu[xi]->name);
	    }
    }
...
int mdockiconui_menu_scroll (TWidget *this, int dir)
{
    _MAKETHIS;
    int oldtop, oldsel;
    TTK_SCROLLMOD (dir, 5);
...
int mdockiconui_menu_button (TWidget *this, int button, int time)
{
    _MAKETHIS;
    ttk_menu_item *item;
    TWindow *sub;
	...
	if (item->sub == TTK_MENU_DONOTHING) {
	    ...
	    int r = -1;
	    while (ttk_windows->w->focus->draw == mdockiconui_menu_draw &&
...
int mdockiconui_menu_down (TWidget *this, int button)
{
    return TTK_EV_UNUSED;
}
...
void mdockiconui_menu_free (TWidget *this) 
{
    int i;
    _MAKETHIS;
...
void mdockiconui_menu_clear (TWidget *this) 
{
    int i;
    _MAKETHIS;
    if (data->menu) {
...
TWindow *mdockiconuix_mh_sub (ttk_menu_item *item) 
{
    TWindow *ret = ttk_new_window();

    ttk_add_widget (ret, mdockiconui_new_menu_widget (item->data, ttk_menufont,
...
void *mdockiconuix_md_sub (ttk_menu_item *submenu) 
{
    return (void *)submenu;
}
