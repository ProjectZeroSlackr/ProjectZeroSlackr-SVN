...
static void chopRenderTerrain(CTerrain *ter)
{
	...
	int i=1;
	GrSetGCForeground(chopGC, GR_RGB(0,0,0));
...
static void chopDrawPlayer(int x,int y) /*These are SCREEN coords, not world!*/
{
	...
	GrSetGCForeground(chopGC, GR_RGB(0,0,0));
	GrFillRect(chopBuffer, chopGC, 	x+6,		y+2,		12,		9	); 
	GrFillRect(chopBuffer, chopGC, 	x - 3,		y+6,		23,		3	); 
	GrFillRect(chopBuffer, chopGC, 	x + 10,		y,			2,		3	); 
	GrFillRect(chopBuffer, chopGC, 	x -2,		y+5	,		2,		5	); 
	GrFillRect(chopBuffer, chopGC, 	x +18,		y+5	,		2,		5	); 
	
	GrLine(chopBuffer, chopGC, x,y+iRotorOffset, x+20,y-iRotorOffset); //rotor
...
{ 
	...
	GrSetGCForeground(chopGC, GR_RGB(0,0,0));
	GrFillRect(chopBuffer, chopGC, iPosX,iPosY,mBlock->iSizeX,	mBlock->iSizeY	); 
}
...
static void chopDrawParticle(CParticle *mParticle)
{
	...
	GrSetGCForeground(chopGC, GR_RGB(80,80,80));
	GrFillRect(chopBuffer, chopGC, iPosX,iPosY,3,3); 
...
static void chopDrawScene()
{
	...
	GrSetGCForeground(chopGC, GR_RGB(255,255,255));
