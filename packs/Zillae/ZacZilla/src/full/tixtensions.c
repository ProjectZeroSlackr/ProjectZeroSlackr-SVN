...
TWindow * tix_paste_handler(ttk_menu_item * item)
{
	char fn[1024];
	FILE * fin;
	FILE * fout;
	...
	if (tix_file_clipboard) {
		...
		if (tix_file_clipboard_copy) {
			...
			if (! (buf = (char *)malloc(TIX_FILE_BUFFER_SIZE)) ) {
				new_message_window("malloc for file copy failed\n");
			...
			if (!feof(fin)) {
				new_message_window("fwrite call returned error in file copy");
			}
			if (fin) fclose(fin);
			if (fout) fclose(fout);
			free(buf);
...
TWindow * tix_pastelink_handler(ttk_menu_item * item)
{
	char fn[1024];
	if (tix_file_clipboard) {
		getcwd(fn, 1024);
		...
		if (symlink(tix_file_clipboard, fn)) {
			new_message_window("symlink call returned error");
		}
	}
	return TTK_MENU_UPONE;
}

TWindow * tix_copy_handler(ttk_menu_item * item)
{
...
TWindow * tix_cut_handler(ttk_menu_item * item)
{
	if (tix_file_clipboard) free(tix_file_clipboard);
	if (tix_file_clipboard_name) free(tix_file_clipboard_name);
	tix_file_clipboard = (char *)malloc(1024);
...
void init_ti_extensions()
{
	...
	ttk_browser_add_action (tix_true, &tix_pastelink_menuitem);
}
