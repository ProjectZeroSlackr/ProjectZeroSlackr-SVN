...
static TWindow *mcp_wid;
static TWidget *mcp_widget;
...
static void mcp_draw_percent(TWidget *wid, ttk_surface srf, int per)
{
...
static void mcp_place_text(TWidget *wid, ttk_surface srf, void *text,
		int mod, int num)
{
	int w, h;
...
static void mcp_draw_volume(TWidget *wid, ttk_surface srf)
{
	char tmp[20];
	int w, h;
...
static void mcp_draw_position(TWidget *wid, ttk_surface srf)
{
	char tmp[20];
	int w, h;
...
static void mcp_draw_screen(TWidget *wid, ttk_surface srf)
{
	int posmod, i = 0;
...
static int mpc_down(TWidget *this,int button)
{
		switch (button) {
...
static int mpc_scroll(TWidget *this,int dir)
{
		switch (multibar.state) {
		case POSITION:
		...
		case VOLUME_S:
			status.volume += dir * 2;
			status.volume -= status.volume % (dir * 2);
			BOUNDS(status.volume, 0, 100);
			mpdc_change_volume(mpdz, status.volume);
			break;
		case SEEK:
			if (accel.rate && (!(dir < 0 && accel.rate < 0) &&
					!(dir > 0 && accel.rate > 0))) {
				accel.step = 1;
				accel.rate = 0;
			}
			accel.rate += dir;
			multibar.seek_time += dir * (4 * accel.step);
...
static int mpc_button(TWidget *this,int button,int time)
{
		switch (button) {
		case TTK_BUTTON_NEXT:
...
TWindow *mpd_currently_playing()
{
	...
	mcp_wid = ret = ttk_new_window(_("Now Playing"));
	mcp_widget = ttk_new_widget(0,0);
	mpc_widget->focusable=1;
	mpc_widget->draw=mpc_draw_window;
	mpc_widget->down=mpc_down;
	mpc_widget->button=mpc_button;
	mpc_widget->scroll=mpc_scroll;
	ttk_add_widget(ret, mpc_widget);
