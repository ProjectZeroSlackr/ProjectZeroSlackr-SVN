...
// KERIPO MOD
// Note: All "menu.conf" messages now refer to MENUCONF_CONFIG_FILE
...
int pz_menuconf_runargs(int argc, char * argv[])
{
	...
	str_case("rm")
		if (argc < 2) {
			pz_error("Too few arguments to %s in " MENUCONF_CONFIG_FILE ".", argv[0]);
		} else if (CHKPWR(argv[1])) {
			pz_error("Cannot modify the Power menu from " MENUCONF_CONFIG_FILE ".");
		} else {
	...
	str_case("mv")
		if (argc < 3) {
			...
			if (item) {
				...
				if (!nitem) {
					pz_error("Could not create menu item %s in " MENUCONF_CONFIG_FILE ".", argv[2]);
				} else {
	...
	str_default
		pz_error("Invalid command %s in " MENUCONF_CONFIG_FILE ".", argv[0]);
	}
	return 0;
}
