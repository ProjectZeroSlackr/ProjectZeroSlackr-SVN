...
{
// KERIPO MOD
// Force symlink to refer to a fixed location rather than relative
// Needed since "file" is only the scheme's file name, not the full path
// Just don't make the SCHEMESDIR too long and have ridiculously long
// scheme file names that total to over 63 characters ; P
// I'll try to write up something that finds the absolute path of the scheme
// file automatically later if I can find such a function. 
    char schemefile[64] = SCHEMESDIR;
    strcat ( schemefile, file );

    if (save) {
// KERIPO MOD
//     	unlink (SCHEMESDIR "default.cs");
//     	symlink (file, SCHEMESDIR "default.cs");
    	unlink (SYMLINKSCHEMEDIR SYMLINKDEFSCHEME);
    	symlink (schemefile, SYMLINKSCHEMEDIR SYMLINKDEFSCHEME);
    }
...
//            ttk_ap_load (SCHEMESDIR "default.cs");
            ttk_ap_load (SYMLINKSCHEMEDIR SYMLINKDEFSCHEME);
        }
...
//         pz_perror ("reading default.cs symlink");
    if (readlink (SYMLINKSCHEMEDIR SYMLINKDEFSCHEME, linktarget, 256) < 0) {
        pz_perror ("reading " SYMLINKDEFSCHEME " symlink");
        linktarget[0] = 0;
    }
...
//     	if (strchr (d->d_name, '.') && !strcmp (strrchr (d->d_name, '.'), ".cs") && !!strcmp (d->d_name, "default.cs")) {
    	if (strchr (d->d_name, '.') && !strcmp (strrchr (d->d_name, '.'), ".cs") && !!strcmp (d->d_name, SYMLINKDEFSCHEME)) {
