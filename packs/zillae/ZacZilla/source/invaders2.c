...
static int invaders2_do_draw(TWidget* wid, ttk_surface srf)
{
	invaders2_srf = srf;
	starfield_draw();
	aliens_draw(0);
...
static void update_score()
{
	...
	//ttk_text(score_srf,2,2,ttk_menufont,ttk_makecol(BLACK),s);
	if(score > high_score)
		high_score = score;
	...
	//ttk_text(score_srf,invaders2_srf->w-2,2,ttk_menufont,ttk_makecol(BLACK),s);
...
int ret=0;
	if(game_status == GAME_STATUS_PLAY) { /* if in play */
			...
			me_posx += dir; //if this is negative, subtract
...
static int invaders2_down(TWidget *this,int button)
{
		if(game_status == GAME_STATUS_PLAY) { /* if in play */
			switch(button) {
...
TWindow *new_invaders2_window()
{
	...
	/* load graphics */
	aliens_gfx  = ttk_load_image ("Data/invaders2/alien.png");
	player_gfx = ttk_load_image ("Data/invaders2/player.png");
	die_gfx[0] = ttk_load_image ("Data/invaders2/die0.png");
	die_gfx[1] = ttk_load_image ("Data/invaders2/die1.png");
	die_gfx[2] = ttk_load_image ("Data/invaders2/die2.png");
	die_gfx[3] = ttk_load_image ("Data/invaders2/die3.png");
