...
static char pathimage[]="Data/KMData/pict.gif";
static char pathmap[]="Data/KMData/mapdata/stage1.txt";
...
// KERIPO MOD
	...
	if (training==0){sprintf(pathmap,"Data/KMData/mapdata/stage%d.txt",wmenukey.STAGE);}
	//else if (training==1){tranum++;sprintf(pathmap,"/etc/KMData/tra/tra%d.txt",tranum);}
	else if (training==1){tranum++;sprintf(pathmap,"Data/KMData/tra/tra%d.txt",tranum);}
	switch (wmenukey.KEY){
...
void new_keyman_window( void )
{
	...
	if (!(kmimage_id=GrLoadImageFromFile("Data/KMData/pict.gif",0))){
		GAMEN=NODATA;
