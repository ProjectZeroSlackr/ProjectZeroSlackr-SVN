...
int ti_serial_abort(TWidget * this, int ch) 
{
	if (ch == TTK_BUTTON_ACTION) {
		new_message_window (_("I'd suggest picking another text input method before you try that again."));
		ttk_input_end();
		return TTK_EV_CLICK;
	}
...
ipod_set_setting(50,ti_selected_tim);
	return 0;
}
...
{
	if(item->choice==0)
	ti_select(item->choice);
	else
	ti_select(item->choice+1);
	new_message_window(item->choice);
	return TTK_MENU_UPONE;
}

TWindow * ti_ttkselect0(ttk_menu_item * item)
{
...
TWindow * ti_ttkselect1(ttk_menu_item * item)
{
...
TWindow * ti_ttkselect2(ttk_menu_item * item)
{
...
int ti_register(TWidget * (* create)(), TWidget * (* ncreate)(), char * name, int id)
{
	...
	//ttk_menu_add_ttkh(menupath, ti_ttkselect, (void *)(long)id);
...
void ti_init()
{
	...
	if (ipod_get_setting( 50)) {
		ti_selected_tim = ti_saved_tim = ipod_get_setting(50);
	} else {
