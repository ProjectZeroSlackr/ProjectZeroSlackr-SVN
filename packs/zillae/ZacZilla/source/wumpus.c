...
typedef struct myGlobals {
        ...
        TWindow * window;      /* our window */
        TWidget * widget;      /* our display widget */
...
/*			if( !pz_dialog(  _("New Game?"), 
				_("You have an unfinished game; Continue?" ),
					2, 0, _("Yes"), _("No"))) */{
				glob.initialized = 0; /* state 1 not 2 */
...
static void wumpus_draw( TWidget * widget, ttk_surface srf )
{
...
static int wumpus_scroll(TWidget *this,int dir)
{
		if( glob.game.health ) {
...
static int wumpus_down(TWidget *this,int button)
{
                switch( button ) {
...
static int wumpus_button(TWidget *this,int button,int time)
{
                switch( button ) {     
		case( TTK_BUTTON_HOLD ):
                        glob.paused = 0;
...
static int wumpus_timer(TWidget *this)
{
		if( !glob.paused ) {
...
TWindow * wumpus_new_game( void )
{
	...
	/* set up our window */
	glob.window = ttk_new_window( "Wumpus");
	glob.w = glob.window->w;
	glob.h = glob.window->h;
	glob.widget = ttk_new_widget(0,0);
	glob.widget->focusable=1;
	glob.widget->draw=wumpus_draw;
	glob.widget->timer=wumpus_timer;
	glob.widget->down=wumpus_down;
	glob.widget->button=wumpus_button;
	glob.widget->scroll=wumpus_scroll;
	ttk_add_widget( glob.window, glob.widget);
	...
	ttk_widget_set_timer( glob.widget, 300 );
	
	/* finish and return it */
	ttk_show_window( glob.window );
	return 0;
}
