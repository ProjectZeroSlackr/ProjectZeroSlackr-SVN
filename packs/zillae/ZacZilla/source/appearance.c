...
// KERIPO MOD
    ...
    if (save) {
	unlink ("/sandbox/" "zaczilla-default.cs");
	symlink (schemefile, "/sandbox/" "zaczilla-default.cs");
    }
...
void appearance_reset_color_scheme() 
{
    ...
    ttk_ap_load ("/sandbox/" "zaczilla-default.cs");
}
...
const char *appearance_get_color_scheme (void)
{
    ...
    readlink ("/sandbox/" "zaczilla-default.cs", link, 256);
    return link;
}
...
TWindow *appearance_select_color_scheme (ttk_menu_item *item) 
{
	...
	if (strchr (d->d_name, '.') && !strcmp (strrchr (d->d_name, '.'), ".cs") && !!strcmp (d->d_name, "zaczilla-default.cs")) {
