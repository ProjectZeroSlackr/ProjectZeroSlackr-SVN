...
static char *cnames[]={"ZacZilla", "by Zack Frey aka zacaj",
		"IPL DEVELOPERS", 
                              ...
                              const int fillbg)
{
	...
	if (fillbg) {
		...
		GrSetGCBackground(credits_gc, GR_RGB(0,0,0));
	} else {
...
{
	GrSetGCForeground(credits_gc, GR_RGB(0,0,0));
	if (exlcude_stream) {
...
static void credits_stream_push(void)
{
	...
	for (i = 0; i < cols; i++) {
		...
		if ((i > cnt2 && i <= cnt2 + txt_len && txt_status >= CENTRE) ||
		    ...
		    colour = PHOTO ? GR_RGB(0, (int) rand() % 35 + 220, 0) : GR_RGB(160,160,160);
		} else {
			colour = PHOTO ? GR_RGB(0, (int) rand() % 100 + 60, 0) : GR_RGB(80,80,80);
		}
