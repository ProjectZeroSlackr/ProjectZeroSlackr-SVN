...
#define MIN_CELL_WIDTH(t) ((t->w < 190) ? (50) : (60))
#define MIN_CELL_HEIGHT(t) ((t->h < 112) ? (52) : (56))
...
{
    int vi, xi, nc, nr, ih, iw, ofs, iofs;
	...
	nc = (this->w - 10) / MIN_CELL_WIDTH(this);
	nr = this->h / MIN_CELL_HEIGHT(this);
	ih = data->itemheight & 0xFFFF;
	iw = data->itemheight >> 16;
    iofs = (ih - 50) / 2;
    ofs = iofs + 33;
	...
	data->itemsrf[xi] = ttk_new_surface (iw, ih, ttk_screen->bpp);
	ttk_fillrect (data->itemsrf[xi], 0, 0, iw, ih, menu_bg_color );
        ...
        ttk_text (data->itemsrf[xi], data->font, (data->menu[xi]->linewidth - ttk_text_width(data->font, truncname))/2,
        		ofs, menu_fg_color, truncname);
        iconui_draw_icon (data->itemsrf[xi], (char *)data->menu[xi]->name, (data->menu[xi]->linewidth - 32)/2, iofs);

	// Selected
	if (data->itemsrfI[xi]) ttk_free_surface (data->itemsrfI[xi]);
	data->itemsrfI[xi] = ttk_new_surface (iw, ih, ttk_screen->bpp);
	if (ttk_ap_getx("menu.selbg")->type & TTK_AP_GRADIENT &&
	    ...
	    ttk_ap_rect (data->itemsrfI[xi], ttk_ap_getx ("menu.selbg"), 0,
				0, iw + ttk_ap_getx("menu.selbg")->rounding, ih); 
	else
	    ttk_fillrect (data->itemsrfI[xi], 0, 0, iw, ih, menu_selbg_color );
        ...
        ttk_text (data->itemsrfI[xi], data->font, (data->menu[xi]->linewidth - ttk_text_width(data->font, truncname))/2,
        		ofs, menu_selfg_color, truncname);
        
/*
        if (data->i18nable)
            ttk_text (data->itemsrfI[xi], data->font, 
            (data->menu[xi]->linewidth - ttk_text_width(data->font, gettext (data->menu[xi]->name) ))/2, ofs, 
			menu_selfg_color, gettext (data->menu[xi]->name));
        else
            ttk_text (data->itemsrfI[xi], data->font, 
            (data->menu[xi]->linewidth - ttk_text_width(data->font, data->menu[xi]->name ))/2, ofs, 
			menu_selfg_color, data->menu[xi]->name);
*/
		
		iconui_draw_icon (data->itemsrfI[xi], (char *)data->menu[xi]->name, (data->menu[xi]->linewidth - 32)/2, iofs);
    }
}



ttk_menu_item *bigicon2ui_menu_get_item (TWidget *this, int xi) 
{
    _MAKETHIS;
...
ttk_menu_item *bigicon2ui_menu_get_item_called (TWidget *this, const char *s)
{
    int xi = 0;
    _MAKETHIS;
    if (data->menu) {
...
ttk_menu_item *bigicon2ui_menu_get_selected_item (TWidget *this) 
{
    _MAKETHIS;
...
void bigicon2ui_menu_item_updated (TWidget *this, ttk_menu_item *p)
{
    _MAKETHIS;
    int i;
...
void bigicon2ui_menu_remove_by_name (TWidget *this, const char *name)
{
    _MAKETHIS;
    int xi;
...
void bigicon2ui_menu_remove_by_ptr (TWidget *this, ttk_menu_item *item) 
{
    _MAKETHIS;
...
void bigicon2ui_menu_remove (TWidget *this, int xi) 
{
    _MAKETHIS;
...
void bigicon2ui_menu_insert (TWidget *this, ttk_menu_item *item, int xi) 
{
    _MAKETHIS;
    
    if (xi >= data->items) {
        bigicon2ui_menu_append (this, item);
        return;
    }
    ...
    bigicon2ui_menu_item_updated (this, item);
    MakeVIXI (this);
    item->menudata = data;
...
void bigicon2ui_menu_append (TWidget *this, ttk_menu_item *item) 
{
    _MAKETHIS;
...
void bigicon2ui_menu_sort_my_way (TWidget *this, int (*cmp)(const void *, const void *))
{
    _MAKETHIS;
...
void bigicon2ui_menu_sort (TWidget *this) 
{
    _MAKETHIS;
    if (data->i18nable)
        bigicon2ui_menu_sort_my_way (this, sort_compare_i18n);
    else
        bigicon2ui_menu_sort_my_way (this, sort_compare);
}

void bigicon2ui_menu_updated (TWidget *this)
{
    ttk_menu_item *p, **q;
    _MAKETHIS;
    ...
    q = data->menu;
    while (p && p->name) {
	bigicon2ui_menu_item_updated (this, p);
	p->menudata = data;
...
int bigicon2ui_menu_frame (TWidget *this) 
{   
...
void bigicon2ui_menu_flash (ttk_menu_item *item, int n) 
{
    item->textflash = 2*n;
}


TWidget *bigicon2ui_new_menu_widget (ttk_menu_item *items, ttk_font font, int w, int h)
{
	int nc, nr, cw, rh;
    ttk_menu_item *p;
    ...
    ret->data = data;
    ret->focusable = 1;
    ret->draw = bigicon2ui_menu_draw;
    ret->frame = bigicon2ui_menu_frame;
    ret->down = bigicon2ui_menu_down;
    ret->button = bigicon2ui_menu_button;
    ret->scroll = bigicon2ui_menu_scroll;
    ret->destroy = bigicon2ui_menu_free;

	nc = (w-10) / MIN_CELL_WIDTH(ret);
	nr = h / MIN_CELL_HEIGHT(ret);
	cw = (w-10) / nc;
	rh = h / nr;
    data->visible = nc*nr;
    data->itemheight = (rh | (cw << 16));
    data->mlist = items;
    data->menu = 0;
    data->font = font;
...
void bigicon2ui_menu_set_closeable (TWidget *this, int closeable) 
{
    _MAKETHIS;
    data->closeable = closeable;
}
void bigicon2ui_menu_set_i18nable (TWidget *this, int i18nable) 
{
    _MAKETHIS;
    data->i18nable = i18nable;
}

void bigicon2ui_menu_draw (TWidget *this, ttk_surface srf)
{
    _MAKETHIS;
    int i, y = this->y;
    int nc = (this->w - 10) / MIN_CELL_WIDTH(this);
    int nr = this->h / MIN_CELL_HEIGHT(this);
    int iw = (this->w - 10) / nc;
	int ih = this->h / nr;
    int ofs = ((ih - 50) / 2) + 42;
    int nivis = 0;
    int spos, sheight;
    int vi, xi;
   	int top = data->top; /* ((nr < 2) ? (data->top) : ( nc * ((data->top+nc-1) / nc)  )); */
   	int sel = data->sel; /* ((nr < 2) ? (data->sel) : ( data->sel - (top - data->top) )); */
	...
	cw = (this->w - 10) / nc;
	rh = this->h / nr;
	data->font = iconui_font;
	data->visible = nc*nr;
	data->itemheight = (rh | (cw << 16));
	ih = rh; iw = cw;
	for (i = 0; i < data->items; i++)
	    bigicon2ui_menu_item_updated (this, data->menu[i]);
	render (this, top, data->visible);
	data->epoch = ttk_epoch;
    }
    ...
    for (vi = top, xi = data->xivi[top];
	 data->menu[xi] && vi < MIN (top + data->visible, data->vitems);
	 vi++, xi = data->xivi[vi]) {
	ttk_color col;
        int selected = (vi == top + sel);
	...
	if (vi == top + sel && !(data->menu[xi]->textflash % 2)) {
	    if (!data->itemsrfI[xi]) {
		fprintf (stderr, "Null pointer (I)\n");
		abort();
	    }
	    
	    ttk_blit_image (data->itemsrfI[xi], srf, this->x + iw*((vi-top)%nc), y + ih*((vi-top)/nc) );
	    ...
	    ttk_blit_image (data->itemsrf[xi], srf, this->x + iw*((vi-top)%nc), y + ih*((vi-top)/nc) );
	    col = ttk_ap_getx ("menu.fg") -> color;
	}

	if (data->menu[xi]->choices) {
		char * truncname = strdup(data->menu[xi]->choices[data->menu[xi]->choice]);
        if (ttk_text_width (data->font, truncname) > data->menu[xi]->linewidth) {
            sprintf (truncname + strlen (truncname) - 4, "...");
            while (strlen (truncname) > 3 && ttk_text_width (data->font, truncname) > data->menu[xi]->linewidth) {
                sprintf (truncname + strlen (truncname) - 5, "...");
	    ...
	    ttk_text (srf, data->font, this->x + iw*((vi-top)%nc) + (iw -
		      ttk_text_width (data->font, truncname))/2,
		      y + ofs + ih*((vi-top)/nc), col, truncname);
		free(truncname);
	}
...
*/
	...
	spos = top * (this->h - 2*ttk_ap_getx ("header.line") -> spacing) / nivis - 1;
...
static int intceil(int a, int m);
static int intfloor(int a, int m);

static int intceil(int a, int m)
{
	if (a < 0) return -intfloor(-a,m);
	if (a % m) a += m - (a % m);
	return a;
}
...
static int intfloor(int a, int m)
{
	if (a < 0) return -intceil(-a,m);
	if (a % m) a -= (a % m);
	return a;
}

int bigicon2ui_menu_scroll (TWidget *this, int dir)
{
    _MAKETHIS;
    int nc = (this->w - 10) / MIN_CELL_WIDTH(this);
    int nr = this->h / MIN_CELL_HEIGHT(this);
	...
	data->top += intceil(data->sel - data->visible + 1, nc);
	data->sel -= intceil(data->sel - data->visible + 1, nc);
    }
    if (data->sel < 0) {
	data->top += intfloor(data->sel, nc); // actually subtracts; sel is negative
	data->sel -= intfloor(data->sel, nc);
    }
	...
	data->sel -= intceil(data->vitems - data->visible, nc) - data->top;
	data->top  = intceil(data->vitems - data->visible, nc);
    }
    if (data->top < 0) {
    ...
    if (data->sel >= MIN (data->visible, data->vitems - data->top)) {
	data->sel = MIN (data->visible, data->vitems - data->top) - 1;
    }
...
int bigicon2ui_menu_button (TWidget *this, int button, int time)
{
    _MAKETHIS;
    ttk_menu_item *item;
    TWindow *sub;
	...
	if (item->sub == TTK_MENU_DONOTHING) {
	    ...
	    int r = -1;
	    while (ttk_windows->w->focus->draw == bigicon2ui_menu_draw &&
...
int bigicon2ui_menu_down (TWidget *this, int button)
{
    return TTK_EV_UNUSED;
}
...
void bigicon2ui_menu_free (TWidget *this) 
{
    int i;
    _MAKETHIS;
...
void bigicon2ui_menu_clear (TWidget *this) 
{
    int i;
    _MAKETHIS;
    if (data->menu) {
...
TWindow *bigicon2uix_mh_sub (ttk_menu_item *item) 
{
    TWindow *ret = ttk_new_window();

    ttk_add_widget (ret, bigicon2ui_new_menu_widget (item->data, iconui_font,
...
void *bigicon2uix_md_sub (ttk_menu_item *submenu) 
{
    return (void *)submenu;
}
