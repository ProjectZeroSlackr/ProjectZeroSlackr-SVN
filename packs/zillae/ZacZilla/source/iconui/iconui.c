...
#define COLOR_SCHEME_PATH "/sandbox/zaczilla-default.cs"
#endif
...
ttk_surface iconui_get_icon(char * name)
{
	...
	if (icons_na) {
		sprintf(path, "Data/icons/Default.png");
		if (!access(path, R_OK)) {
			if (iconcache) {
				iconsrf = iconui_cache_set(s, ttk_load_image((const char *)path) );
			} else {
	...
	strcpy(path, "Data/icons/Default.png");
	if (!access(path, R_OK)) {
		if (iconcache) {
			iconsrf = iconui_cache_set(s, ttk_load_image((const char *)path) );
		} else {
