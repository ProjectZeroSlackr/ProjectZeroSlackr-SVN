...
ttk_menu_item *dockiconui_menu_get_item (TWidget *this, int xi) 
{
    _MAKETHIS;
...
ttk_menu_item *dockiconui_menu_get_item_called (TWidget *this, const char *s)
{
    int xi = 0;
    _MAKETHIS;
    if (data->menu) {
...
ttk_menu_item *dockiconui_menu_get_selected_item (TWidget *this) 
{
    _MAKETHIS;
...
void dockiconui_menu_item_updated (TWidget *this, ttk_menu_item *p)
{
    _MAKETHIS;
    int i;
...
void dockiconui_menu_remove_by_name (TWidget *this, const char *name)
{
    _MAKETHIS;
    int xi;
...
void dockiconui_menu_remove_by_ptr (TWidget *this, ttk_menu_item *item) 
{
    _MAKETHIS;
...
void dockiconui_menu_remove (TWidget *this, int xi) 
{
    _MAKETHIS;
...
void dockiconui_menu_insert (TWidget *this, ttk_menu_item *item, int xi) 
{
    _MAKETHIS;
    
    if (xi >= data->items) {
        dockiconui_menu_append (this, item);
        return;
    }
    ...
    dockiconui_menu_item_updated (this, item);
    MakeVIXI (this);
    item->menudata = data;
...
void dockiconui_menu_append (TWidget *this, ttk_menu_item *item) 
{
    _MAKETHIS;
...
void dockiconui_menu_sort_my_way (TWidget *this, int (*cmp)(const void *, const void *))
{
    _MAKETHIS;
...
void dockiconui_menu_sort (TWidget *this) 
{
    _MAKETHIS;
    if (data->i18nable)
        dockiconui_menu_sort_my_way (this, sort_compare_i18n);
    else
        dockiconui_menu_sort_my_way (this, sort_compare);
}

void dockiconui_menu_updated (TWidget *this)
{
    ttk_menu_item *p, **q;
    _MAKETHIS;
    ...
    q = data->menu;
    while (p && p->name) {
	dockiconui_menu_item_updated (this, p);
	p->menudata = data;
...
int dockiconui_menu_frame (TWidget *this) 
{   
...
void dockiconui_menu_flash (ttk_menu_item *item, int n) 
{
    item->textflash = 2*n;
}
...
TWidget *dockiconui_new_menu_widget (ttk_menu_item *items, ttk_font font, int w, int h)
{
    ...
    ret->data = data;
    ret->focusable = 1;
    ret->draw = dockiconui_menu_draw;
    ret->frame = dockiconui_menu_frame;
    ret->down = dockiconui_menu_down;
    ret->button = dockiconui_menu_button;
    ret->scroll = dockiconui_menu_scroll;
    ret->destroy = dockiconui_menu_free;
...
void dockiconui_menu_set_closeable (TWidget *this, int closeable) 
{
    _MAKETHIS;
    data->closeable = closeable;
}
void dockiconui_menu_set_i18nable (TWidget *this, int i18nable) 
{
    _MAKETHIS;
    data->i18nable = i18nable;
}

void dockiconui_menu_draw (TWidget *this, ttk_surface srf)
{
    _MAKETHIS;
    int i, x = this->x + (this->w - (MIN(data->visible, data->vitems))*data->itemheight)/2, y = this->y + this->h - data->itemheight;
    int ofs = (data->itemheight - 32) / 2;
    int nivis = 0;
    int spos, sheight;
    int vi, xi;
    int selxi = -1, selx = -1;
	...
	for (i = 0; i < data->items; i++)
	    dockiconui_menu_item_updated (this, data->menu[i]);
	render (this, data->top, data->visible);
	data->epoch = ttk_epoch;
    }
...
int dockiconui_menu_scroll (TWidget *this, int dir)
{
    _MAKETHIS;
    int oldtop, oldsel;
    TTK_SCROLLMOD (dir, 5);
...
int dockiconui_menu_button (TWidget *this, int button, int time)
{
    _MAKETHIS;
    ttk_menu_item *item;
    TWindow *sub;
	...
	if (item->sub == TTK_MENU_DONOTHING) {
	    ...
	    int r = -1;
	    while (ttk_windows->w->focus->draw == dockiconui_menu_draw &&
...
int dockiconui_menu_down (TWidget *this, int button)
{
    return TTK_EV_UNUSED;
}
...
void dockiconui_menu_free (TWidget *this) 
{
    int i;
    _MAKETHIS;
...
void dockiconui_menu_clear (TWidget *this) 
{
    int i;
    _MAKETHIS;
    if (data->menu) {
...
TWindow *dockiconuix_mh_sub (ttk_menu_item *item) 
{
    TWindow *ret = ttk_new_window();

    ttk_add_widget (ret, dockiconui_new_menu_widget (item->data, ttk_menufont,
...
void *dockiconuix_md_sub (ttk_menu_item *submenu) 
{
    return (void *)submenu;
}
