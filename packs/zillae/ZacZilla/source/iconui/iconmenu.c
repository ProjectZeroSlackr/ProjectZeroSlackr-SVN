...
static void render (TWidget *this, int first, int n)
{
	...
	data->itemsrf[xi] = ttk_new_surface (data->menu[xi]->textwidth + 26,
					    data->itemheight, ttk_screen->bpp);
	ttk_fillrect (data->itemsrf[xi], 0, 0, data->menu[xi]->textwidth + 26, 
        ...
        ttk_text (data->itemsrf[xi], data->font, 23, ofs, menu_fg_color, truncname);
        iconui_draw_small_icon (data->itemsrf[xi], (char *)data->menu[xi]->name, 3, iofs);

	// Selected
	if (data->itemsrfI[xi]) ttk_free_surface (data->itemsrfI[xi]);
	data->itemsrfI[xi] = ttk_new_surface (data->menu[xi]->textwidth + 26,
					     data->itemheight, ttk_screen->bpp);
	if (ttk_ap_getx("menu.selbg")->type & TTK_AP_GRADIENT &&
	    ...
	    ttk_ap_rect (data->itemsrfI[xi], ttk_ap_getx ("menu.selbg"), 0,
				0, data->menu[xi]->textwidth + 26 +
				ttk_ap_getx("menu.selbg")->rounding, data->itemheight); 
	else
	    ttk_fillrect (data->itemsrfI[xi], 0, 0, data->menu[xi]->textwidth + 26,
				data->itemheight, menu_selbg_color );
        if (data->i18nable)
            ttk_text (data->itemsrfI[xi], data->font, 23, ofs, 
			menu_selfg_color, gettext (data->menu[xi]->name));
        else
            ttk_text (data->itemsrfI[xi], data->font, 23, ofs, 
			menu_selfg_color, data->menu[xi]->name);
		
		iconui_draw_small_icon (data->itemsrfI[xi], (char *)data->menu[xi]->name, 3, iofs);
    }
}



ttk_menu_item *iconui_menu_get_item (TWidget *this, int xi) 
{
    _MAKETHIS;
...
ttk_menu_item *iconui_menu_get_item_called (TWidget *this, const char *s)
{
    int xi = 0;
    _MAKETHIS;
    if (data->menu) {
...
ttk_menu_item *iconui_menu_get_selected_item (TWidget *this) 
{
    _MAKETHIS;
...
void iconui_menu_item_updated (TWidget *this, ttk_menu_item *p)
{
    _MAKETHIS;
    int i;
...
void iconui_menu_remove_by_name (TWidget *this, const char *name)
{
    _MAKETHIS;
    int xi;
...
void iconui_menu_remove_by_ptr (TWidget *this, ttk_menu_item *item) 
{
    _MAKETHIS;
...
void iconui_menu_remove (TWidget *this, int xi) 
{
    _MAKETHIS;
...
void iconui_menu_insert (TWidget *this, ttk_menu_item *item, int xi) 
{
    _MAKETHIS;
    
    if (xi >= data->items) {
        iconui_menu_append (this, item);
        return;
    }
    ...
    iconui_menu_item_updated (this, item);
    MakeVIXI (this);
    item->menudata = data;
...
void iconui_menu_append (TWidget *this, ttk_menu_item *item) 
{
    _MAKETHIS;
...
void iconui_menu_sort_my_way (TWidget *this, int (*cmp)(const void *, const void *))
{
    _MAKETHIS;
...
void iconui_menu_sort (TWidget *this) 
{
    _MAKETHIS;
    if (data->i18nable)
        iconui_menu_sort_my_way (this, sort_compare_i18n);
    else
        iconui_menu_sort_my_way (this, sort_compare);
}

void iconui_menu_updated (TWidget *this)
{
    ttk_menu_item *p, **q;
    _MAKETHIS;
    ...
    q = data->menu;
    while (p && p->name) {
	iconui_menu_item_updated (this, p);
	p->menudata = data;
...
int iconui_menu_frame (TWidget *this) 
{   
...
void iconui_menu_flash (ttk_menu_item *item, int n) 
{
    item->textflash = 2*n;
}
...
TWidget *iconui_new_menu_widget (ttk_menu_item *items, ttk_font font, int w, int h)
{
    ...
    ret->data = data;
    ret->focusable = 1;
    ret->draw = iconui_menu_draw;
    ret->frame = iconui_menu_frame;
    ret->down = iconui_menu_down;
    ret->button = iconui_menu_button;
    ret->scroll = iconui_menu_scroll;
    ret->destroy = iconui_menu_free;
...
void iconui_menu_set_closeable (TWidget *this, int closeable) 
{
    _MAKETHIS;
    data->closeable = closeable;
}
void iconui_menu_set_i18nable (TWidget *this, int i18nable) 
{
    _MAKETHIS;
    data->i18nable = i18nable;
}

void iconui_menu_draw (TWidget *this, ttk_surface srf)
{
    _MAKETHIS;
    int i, y = this->y;
	...
	for (i = 0; i < data->items; i++)
	    iconui_menu_item_updated (this, data->menu[i]);
	render (this, data->top, data->visible);
	data->epoch = ttk_epoch;
    }
...
int iconui_menu_scroll (TWidget *this, int dir)
{
    _MAKETHIS;
    int oldtop, oldsel;
    TTK_SCROLLMOD (dir, 5);
...
int iconui_menu_button (TWidget *this, int button, int time)
{
    _MAKETHIS;
    ttk_menu_item *item;
    TWindow *sub;
	...
	if (item->sub == TTK_MENU_DONOTHING) {
	    ...
	    int r = -1;
	    while (ttk_windows->w->focus->draw == iconui_menu_draw &&
...
int iconui_menu_down (TWidget *this, int button)
{
    return TTK_EV_UNUSED;
}
...
void iconui_menu_free (TWidget *this) 
{
    int i;
    _MAKETHIS;
...
void iconui_menu_clear (TWidget *this) 
{
    int i;
    _MAKETHIS;
    if (data->menu) {
...
TWindow *iconuix_mh_sub (ttk_menu_item *item) 
{
    TWindow *ret = ttk_new_window();

    ttk_add_widget (ret, iconui_new_menu_widget (item->data, ttk_menufont,
...
void *iconuix_md_sub (ttk_menu_item *submenu) 
{
    return (void *)submenu;
}
