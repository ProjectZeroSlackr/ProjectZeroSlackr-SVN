...
#define HARD_RESET_NOW() sansa_hard_reset()
...
#define DMA_HANDLER	(sansa_dma_handler_t *)0x40000014

#define DMA_BASE	0x40000018

#define COP_HANDLER	(sansa_dma_handler_t *)0x4001501C
...
extern unsigned sansa_get_hw_version(void);
extern struct sysinfo_t *sansa_get_sysinfo(void);
extern int sansa_is_pp5022();

extern void sansa_i2c_init(void);
extern int sansa_i2c_send_bytes(unsigned int addr, unsigned int len, unsigned char *data);
extern int sansa_i2c_send(unsigned int addr, int data0, int data1);
extern int sansa_i2c_send_byte(unsigned int addr, int data0);
extern int sansa_i2c_read_byte(unsigned int addr, unsigned int *data);

extern void sansa_i2c_lock();
extern void sansa_i2c_unlock();

extern void sansa_serial_init(void);

typedef void (*sansa_dma_handler_t)(void);
typedef void (*sansa_cop_handler_t)(void);


extern void sansa_set_process_dma(sansa_dma_handler_t new_handler);
extern void sansa_set_handle_cop(sansa_cop_handler_t new_handler);
