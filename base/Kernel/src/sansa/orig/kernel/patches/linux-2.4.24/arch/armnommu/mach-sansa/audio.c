/*
 * audio.c - audio driver for Sansa e200
 *
 * Copyright (c) 2008 Sebastian Duell <Sebastian.Duell@gmx.de>
 */
...
static int sansaaudio_isopen;
// static int sansaaudio_power_state;
static devfs_handle_t dsp_devfs_handle, mixer_devfs_handle;
static int sansa_sample_rate = 44100;
static volatile int *sansaaudio_stereo = (int *)DMA_STEREO;
static int sansa_mic_boost = 100;
static int sansa_line_level = 0x17;
static int sansa_pcm_level = 60;	
static int sansa_active_rec = SOUND_MASK_MIC;
...
static void as3514_write(unsigned int reg, unsigned int value)
{
   ...
   sansa_i2c_lock();
   if(sansa_i2c_send_bytes(AS3514_I2C_ADDR, 2, data) != 0)
   {
...
{
//	sansa_sample_rate = rate;
	return sansa_sample_rate;
}
...
static int sansaaudio_open(struct inode *inode, struct file *filep)
{
	volatile int *r_off = (int *)DMA_READ_OFF;
	volatile int *w_off = (int *)DMA_WRITE_OFF;
	volatile int *dma_active = (int *)DMA_ACTIVE;

	if (sansaaudio_isopen) {
		return -EBUSY;
	}
	...
	*sansaaudio_stereo = 0;
	...
	if (filep->f_mode & FMODE_WRITE) {
		...
		sansa_set_process_dma(i2s_pp5020_pb_dma);
	...
	if (filep->f_mode & FMODE_READ) {
		...
		if (sansa_active_rec == SOUND_MASK_LINE) {
			codec_activate_linein();
		} else if (sansa_active_rec == SOUND_MASK_MIC) {
			codec_activate_mic();
		}
	}
	return 0;
...
static void sansaaudio_txdrain(void)
{
	volatile int *r_off = (int *)DMA_READ_OFF;
	volatile int *w_off = (int *)DMA_WRITE_OFF;
	volatile int *dma_active = (int *)DMA_ACTIVE;
...
static int sansaaudio_close(struct inode *inode, struct file *filep)
{
   if (filep->f_mode & FMODE_WRITE) {
		sansaaudio_txdrain();
...
static ssize_t sansaaudio_write(struct file *filp, const char *buf, size_t count, loff_t *ppos)
{
...
static ssize_t sansaaudio_read(struct file *filp, char *buf, size_t count, loff_t *ppos)
{
	unsigned short *bufsp;
	size_t rem;
	...
	if (!*dma_active) {
		...
		*w_off = 0;
		...
		/* setup I2S FIQ handler */
		sansa_set_process_dma(i2s_pp5020_rec_dma);
...
static int sansaaudio_getspace_ioctl(void *arg)
{
...
static int sansaaudio_ioctl(struct inode *inode, struct file *filp, unsigned int cmd, unsigned long arg)
{
	int rc = 0;
	int val = 0;
	...
	case SNDCTL_DSP_GETOSPACE:
		return sansaaudio_getspace_ioctl((void *)arg);
	}

	return rc;
}

static struct file_operations sansa_dsp_fops = {
	owner: THIS_MODULE,
	llseek:	no_llseek,
	open: sansaaudio_open,
	release: sansaaudio_close,
	write: sansaaudio_write,
	read: sansaaudio_read,
	ioctl: sansaaudio_ioctl,
};

static int sansa_mixer_open(struct inode *inode, struct file *filep)
{
	return 0;
}

static int sansa_mixer_close(struct inode *inode, struct file *filep)
{
	return 0;
}

static int sansa_mixer_ioctl(struct inode *inode, struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	if (_SIOC_DIR(cmd) == _SIOC_READ) {
		...
		case SOUND_MIXER_RECSRC:
			return put_user(sansa_active_rec, (int *)arg);
		...
		case SOUND_MIXER_LINE:	/* line-in jack */
			val = sansa_line_level * 100 / 31;
			val = val << 8 | val;
			return put_user(val, (int *)arg);

		case SOUND_MIXER_MIC:		/* microphone */
			/* 0 or +20dB (mic boost) plus mute */
			return put_user(sansa_mic_boost, (int *)arg);
		}
	}
	else {
		int val, left, right;
		...
		case SOUND_MIXER_RECSRC:
			if (val != sansa_active_rec) {
				if (val == SOUND_MASK_LINE) {
					codec_activate_linein();
				} else if (val == SOUND_MASK_MIC) {
					codec_activate_mic();
				} else {
					val = sansa_active_rec;
				}

				sansa_active_rec = val;
			}
...
static struct file_operations sansa_mixer_fops = {
	owner: THIS_MODULE,
	llseek:	no_llseek,
	open: sansa_mixer_open,
	release: sansa_mixer_close,
	ioctl: sansa_mixer_ioctl,
};
...
static void __init sansaaudio_hw_init(void)
{
   int i;
	/* reset I2C */
	sansa_i2c_init();
...
static int __init sansaaudio_init(void)
{
   printk("sansaaudio: (c) Copyright 2008 Sebastian Duell <Sebastian.Duell@gmx.de>\n");
...
static void __exit sansaaudio_exit(void)
{
...
module_init(sansaaudio_init);
module_exit(sansaaudio_exit);

MODULE_AUTHOR("Sebastian Duell <Sebastian.Duell@gmx.de>");
MODULE_DESCRIPTION("Audio driver for Sansa e200");
MODULE_LICENSE("GPL");
