...
extern int sansa_set_rtc(void);
extern void sansa_timer_interrupt(int irq, void *dev_id, struct pt_regs *regs);
...
{
	set_rtc = sansa_set_rtc;
	timer_irq.handler = sansa_timer_interrupt;

	if ((sansa_get_hw_version() >> 16) > 0x3) {
