/*
 * This is a slightly modified source of the filebrowser module from SVN (July 4, 2007).
 * Ths is built into the ZeroLauncher core and has been modified to allow the opening
 * of the file browser at certain, hardcoded locations. It will be used for rom loading
 * by emulators without their own built-in file browser.
 *
 * ~Keripo
 *
...
TWindow *open_directory( const char *filename, const char *title, int pathshow );
...
PzWindow *new_browser_window()
{
	...
	return open_directory("/", "File Browser", 1);
}
