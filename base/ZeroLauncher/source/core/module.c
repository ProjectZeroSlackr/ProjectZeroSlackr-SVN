...
// KERIPO MOD
// Note:
// To speed up ZeroLauncher startup's module loading,
// I've removed all tags except the name and longname
// That means this breaks compatibility with normal
// modules that, for example, rely on a dependency check
// Tags: author, depsstr, sdepsstr, privesstr
...
//     mkdir (CONFDIR "/modules", 0755);
    mod->cfgpath = malloc (strlen (CONFDIR SUBCONFDIR "/") + strlen (mod->name) + 1);
    sprintf (mod->cfgpath, "%s" SUBCONFDIR "/%s", CONFDIR, mod->name);
    mkdir (CONFDIR, 0755);
    mkdir (CONFDIR SUBCONFDIR, 0755);
...
//    sprintf (fname, "%s/%s.mod.o", mod->mountpt, mod->name);
    sprintf (fname, "%s/%s" MODULESEXTENSION, mod->mountpt, mod->name);
...
//        pz_message_title (_("Warning"), _("No modules. podzilla will probably be very boring."));
        pz_message_title (_("Warning"), _("No modules. " PZNAME " will probably be very boring."));
        return;
    }
...
void pz_module_iterate (void (*fn)(const char *, const char *)) 
{
    PzModule *cur = module_head;
    while (cur) {
        // KERIPO MOD
		//(*fn)(cur->name, cur->longname, cur->author);
		(*fn)(cur->name, cur->longname);
        cur = cur->next;
    }
}
...
void new_fastlaunch_window(PzModule *mod)
{
	// If theres a pre-made launching script, execute it
	// File-check borrowed from PodRead module
	FILE *fp;
	if((fp=fopen(pz_module_get_datapath(mod, "FastLaunch.sh"), "r"))==NULL) {
		pz_message_title("Don't be lazy!", "No \"FastLaunch.sh\" found. Make one!");
	} else {
		pz_exec(pz_module_get_datapath(mod, "FastLaunch.sh"));
	}
}
